...
async function ensurePlayerProfile(userId) {
  await query(
    `INSERT INTO player_profiles (user_id, elo_2v2, wins_2v2, losses_2v2, matches_played_2v2, updated_at)
...
async function setPresence(userId, state, currentPartyId = null, currentMatchId = null) {
  await query(
    `INSERT INTO presence (user_id, state, current_party_id, current_match_id, updated_at)
     VALUES ($1,$2,$3,$4,NOW())
     ON CONFLICT (user_id)
     DO UPDATE SET state=EXCLUDED.state, current_party_id=EXCLUDED.current_party_id, current_match_id=EXCLUDED.current_match_id, updated_at=NOW()`,
...
async function getAccountByUserId(userId) {
  ...
  const result = await query(
    `SELECT u.id, u.steam_id, u.persona_name, u.avatar_full_url, u.profile_url,
            p.elo_2v2, p.wins_2v2, p.losses_2v2, p.matches_played_2v2,
...
async function searchUsersByNickname(q) {
  const result = await query(
    `SELECT u.id, u.persona_name, u.avatar_full_url, p.elo_2v2
     FROM users u
...
async function getHistoryByUserId(userId, limit = 8) {
  const result = await query(
    `SELECT m.public_match_id, m.map_name, m.team_a_score, m.team_b_score, m.winner_team, m.finished_at,
