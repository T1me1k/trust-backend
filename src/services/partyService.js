...
async function getCurrentPartyByUserId(userId) {
  ...
  const invitesResult = await query(
    `SELECT pi.id, pi.status, pi.to_user_id, u.persona_name, pi.created_at, pi.expires_at
     FROM party_invites pi
     JOIN users u ON u.id = pi.to_user_id
...
async function inviteToParty({ actorUserId, targetUserId }) {
  ...
  if (targetParty) throw new Error('target_already_in_party');
...
async function acceptInvite(inviteId, userId) {
  return withTransaction(async (client) => {
    const inviteResult = await client.query(
      `SELECT * FROM party_invites WHERE id = $1 LIMIT 1 FOR UPDATE`,
      [inviteId]
    );
    ...
    const activeParty = await client.query(
      `SELECT pm.party_id FROM party_members pm JOIN parties p ON p.id = pm.party_id WHERE pm.user_id = $1 AND p.status <> 'closed' LIMIT 1`,
      [userId]
    );
...
async function leaveParty(userId) {
  ...
  await withTransaction(async (client) => {
    ...
    if (party.leader_user_id === userId) {
      ...
      await client.query(`UPDATE party_members SET role = CASE WHEN user_id = $2 THEN 'leader' ELSE 'member' END WHERE party_id = $1`, [party.id, newLeaderId]);
    }
  });
...
async function disbandParty(userId) {
  ...
  for (const member of party.members) {
    await setPresence(member.user_id, 'online', null, null);
  }
  return true;
}
